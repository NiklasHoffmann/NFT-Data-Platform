...
const walletDiscoverRequestSchema = z.object({
...
const postHandler = withAuthenticatedRoute(["owners:read", "refresh:token"], async ({ auth }) => {
  ...
  const validatedPayloadResult = walletDiscoverRequestSchema.safeParse(parsedBody.data);

  if (!validatedPayloadResult.success) {
    return buildValidationErrorResponse({
      error: "invalid_wallet_discover_request",
  ...
  const requestedItems = dedupeWalletDiscoverItems(validatedPayload.items);
...
function dedupeWalletDiscoverItems(items: Array<{ chainId: number; contractAddress: string; tokenId: string }>) {
  return [...new Map(
